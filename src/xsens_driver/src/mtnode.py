...
class XSensDriver(object):
    ...
    def __init__(self):
        ...
        frequency = get_param('~frequency', 100)
        self.covar_orien = get_param('~covar_orien',  0.0)
        self.covar_gyro  = get_param('~covar_gyro',   0.0)
        self.covar_acc   = get_param('~covar_acc',    0.0)

        self.covar_orien = self.covar_orien ** 2
        # Change the unit from deg/s/sqrt(hz) to (rad/s)^2
        self.covar_gyro  = math.radians(self.covar_gyro) ** 2 * frequency
        # Change the unit from ug/sqrt(hz) to (m/s^2)^2
        self.covar_acc   = (9.807/(10**6) * self.covar_acc) ** 2  * frequency
    ...
    def spin_once(self):
        ...
        if acc_data:
            if 'Delta v.x' in acc_data: # found delta-v's
                ...
                imu_msg.linear_acceleration.z = convert_coords(acc_data['accX'],
                                                               acc_data['accY'],
                                                               acc_data['accZ'],
                                                               acc_data['frame'])


                imu_msg.linear_acceleration_covariance[0] = self.covar_acc
                imu_msg.linear_acceleration_covariance[3] = self.covar_acc
                imu_msg.linear_acceleration_covariance[6] = self.covar_acc
            else:
        ...
        if gyr_data:
            if 'Delta q0' in gyr_data: # found delta-q's
                ...
                ss_msg.internal.imu.dq.w, \
                ss_msg.internal.imu.dq.x, \
                ss_msg.internal.imu.dq.y, \
                ss_msg.internal.imu.dq.z = convert_quat((gyr_data['Delta q0'],
												 	     gyr_data['Delta q1'],
												 	     gyr_data['Delta q2'],
													     gyr_data['Delta q3']),
													    gyr_data['frame'])
                ...
                imu_msg.angular_velocity.z = convert_coords(gyr_data['gyrX'],
															gyr_data['gyrY'],
														    gyr_data['gyrZ'],
														    gyr_data['frame'])

                imu_msg.angular_velocity_covariance[0] = self.covar_gyro
                imu_msg.angular_velocity_covariance[3] = self.covar_gyro
                imu_msg.angular_velocity_covariance[6] = self.covar_gyro
            else:
        ...
        if orient_data:
            if 'Q0' in orient_data:
                ...
                imu_msg.orientation.z = convert_quat((orient_data['Q0'],
											  		  orient_data['Q1'],
													  orient_data['Q2'],
													  orient_data['Q3']),
													 orient_data['frame'])

                imu_msg.orientation_covariance[0] = self.covar_orien
                imu_msg.orientation_covariance[3] = self.covar_orien
                imu_msg.orientation_covariance[6] = self.covar_orien
